...
public class Aluno {
	...
	public Componente[] componente = new Componente[4];
	...
	public String paraStringNotas() {
		...
		for (int i = 0; i < totNotasCadastradas; i++) {
			dados += componente[i].componente + "\t";
			for (int j = 0; j < 3; j++)
				if (componente[i].nota[j] != -1)
					dados += componente[i].nota[j] + "\t";
				else
					dados += "-\t";
			dados += componente[i].obterMedia() + "\n";
		}
		dados += obterConceito();
		return dados;
	}
