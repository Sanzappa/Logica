...
public class Principal {
	...
	private static void cadastrarNota(int indice) {
		...
		if (quantasNotas < 4) {
			...
			alunos[indice].componente[quantasNotas] = new Componente();
			alunos[indice].componente[quantasNotas].componente = scan.next();
			alunos[indice].componente[quantasNotas].nota[0] = scan.nextFloat();
			alunos[indice].componente[quantasNotas].nota[1] = scan.nextFloat();
			alunos[indice].componente[quantasNotas].nota[2] = scan.nextFloat();
		} else {
	...
	private static int obterQuantasNotas(int indice) {
		int total = 0;
		for (int i = 0; i < alunos[indice].componente.length; i++)
			if (alunos[indice].componente[i] != null)
				total++;
		return total;
	}
